...
df_sentimentLinguisitic = sp.transform_dataframe(df_withSentiment, text_column='posts')

#-------------- 龍Part Start ：訓練集tf-idf前處理（如果已經產生新資料集就不用重複執行）--------

tp = TfidfPreprocessor(max_features=1000)
tfidf_features = tp.generate_features(df)
columns_to_remove = ['tfidf_10', 'tfidf_100', 'tfidf_12', 'tfidf_15', 'tfidf_20']
tfidf_features.drop(columns=columns_to_remove, errors='ignore', inplace=True)

#-------------- 合併Part ----------------------------------------------------------------
if 'mbti_label' in tfidf_features.columns:
    tfidf_features = tfidf_features.drop(columns=['mbti_label'])

df = df.reset_index(drop=True)
tfidf_features = tfidf_features.reset_index(drop=True)

mbti_df = pd.concat([df, tfidf_features], axis=1)
print(mbti_df.head())
mbti_df.to_csv('mbti_preprocess.csv', index=False)
print("資料預處理完成，已儲存至 mbti_preprocess.csv")

mbti_df = pd.read_csv('mbti_preprocess.csv', encoding='utf-8', encoding_errors='ignore')
print(mbti_df.head())
