...
# joblib.dump(tp.vectorizer, "backend/model/tfidf_vectorizer.pkl")
...
print("開始訓練並儲存四個維度的模型")
for dim in ['E/I', 'S/N', 'T/F', 'J/P']:
    print(f"➡ 訓練維度 {dim}（特徵組合 all(set3)，使用 SMOTE）")
    X_train, X_test, y_train, y_test, le = split_data(
        mbti_df, dim, 'all(set3)', feature_dict, smote=True
    )
    ...
    if dim == 'E/I':
        ...
        with open(feature_path, "w") as f:
            for col in X_train.columns:
                f.write(col + "\n")
        print(f"特徵順序已儲存至 {feature_path}")

    # 儲存模型
    dim_code = dim.replace('/', '')
    model_path = os.path.join(output_dir, f"xgb_{dim_code}.pkl")
    joblib.dump(model, model_path)
    print(f"模型儲存至 {model_path}")

    # 儲存對應 LabelEncoder
    le_path = os.path.join(output_dir, f"label_encoder_{dim_code}.pkl")
    joblib.dump(le, le_path)
    print(f"LabelEncoder 儲存至 {le_path}")
